...
def load_model_and_tokenizer(lm, model_dir=None, add_precondition_token=False):
    if lm == 'roberta':
        model_path = model_dir if model_dir else 'roberta-base'
        model = RobertaForMatres.from_pretrained(model_path)
        tokenizer = RobertaTokenizer.from_pretrained(model_path)
        if model_path == 'roberta-base' and add_precondition_token:
            tokenizer.add_tokens(['<pre>'], special_tokens=True)
